...
select round(ST_xmin(st_extent(ext))::numeric, 0) as x0, round(st_xmax(st_extent(ext))::numeric, 0) as x1, round(st_ymin(st_extent(ext))::numeric, 0) as y0, round(st_ymax(st_extent(ext))::numeric, 0) as y1, res, count(*)
