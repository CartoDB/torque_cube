\i pyramid.sql
DROP TABLE IF EXISTS cdb_pyramid.pois CASCADE;
...
SELECT CDB_BuildPyramid('pois', 'the_geom_webmercator', '{val}',
 'floor(((extract(epoch from ($1)."created_at") - ' || min || ')/(' || max || '-' || min || '))*16)::numeric')
FROM tinfo;

-- Print summary of pixels
select count(val) as count0, sum(val) as v0 from  pois;
...
from cdb_pyramid.pois
group by res order by res desc;

-- Count pixels for tile 5,4,3
SELECT count(ext) FROM cdb_pyramid.pois p
 WHERE p.res = ( select max(res) from cdb_pyramid.pois
...
FROM cdb_pyramid.pois i, inp
WHERE i.ext && inp.e GROUP BY inp.e, res ORDER BY inp.e;

-- INSERT a row
INSERT INTO pois (cartodb_id, the_geom_webmercator, created_at, val)
...
-- UPDATE the row
UPDATE pois SET the_geom_webmercator = 'SRID=3857;POINT(300 0)'
WHERE cartodb_id = -1;
...
-- DELETE the rows
DELETE FROM pois WHERE cartodb_id < 0;
...
DROP TABLE IF EXISTS cdb_pyramid.pois CASCADE;
SELECT CDB_BuildPyramid('pois', 'the_geom_webmercator', NULL, NULL);

select res, count(ext) from cdb_pyramid.pois
group by res order by res desc;
